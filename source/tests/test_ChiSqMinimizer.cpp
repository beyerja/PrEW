...
TEST(TestChiSqMinimizer, MultibinConstructor) {
  // Test with two FitBin's that should give chisq=2
  std::function<double()> trivial_prd = []() { return 0; }; // Prediction = 0
  FitBin fb1 (1.0, 1.0, trivial_prd); // Measurement = Unc = 1
  FitBin fb2 (-1.0, 1.0, trivial_prd); // Measurement -1, Unc = 1
  std::vector<FitPar> fit_pars {}; 
